...
import { Album, Photo, SelectedPhoto } from '../../types/albums'
...
const PhotoDetail = () => {
	...
	const getData = async () => {
		if (isAll) return
		return await axios
			.get(`/gallery/albums/photos?query=` + searchParams.get('query'), { withCredentials: true })
			.then(async (item) => {
	...
	if (result) {
		photoItem = <FullImg onClosePhoto={onClosePhoto} selectPhoto={selectedPhoto} photos={photos} />
	}
						...
						<Navbar>
							<Container sx={{ marginTop: '3rem', width: '100%' }}>
								...
								>
									<Box>
										{album ? (
											<CardDetail title={album.name} date={album.date} photoCount={countPhoto} />
										) : null}
									</Box>
									<Box>
											...
											{photos.map((photo: Photo) => {
												...
												return (
													<ImageListItem key={Math.random()}>
														<img
															src={`${
																album?.thumbnail_base_url + '/' + photo.file_name
															}?w=500&fit=crop&auto=format`}
															srcSet={`${
																album?.thumbnail_base_url + '/' + photo.file_name
															}?w=500&fit=crop&auto=format&dpr=2 2x`}
															alt={album?.name}
															loading="lazy"
